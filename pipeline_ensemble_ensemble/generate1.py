...
from box_utils import checkintersection, distinguishing
...
def test():
    ...
    with torch.no_grad():
        ...
        for i in range(len(predict_boxes)):
            ...
            img = original_img.crop((int(predict_boxes[i][0]), int(predict_boxes[i][1]), int(predict_boxes[i][2]) + 1, int(predict_boxes[i][3]) + 1))
            #print(predict_boxes[i][0], predict_boxes[i][1], predict_boxes[i][2], predict_boxes[i][3])
            #print(img.size)
            #print(transforms.ToTensor()(img).size())
            mask.append( predict_img(net=model_ft,
                            full_img=img,
                            scale_factor= 1,
                            out_threshold= 0.5,
                            device=device) )
        ...
        for i in range(len(mask)):
            print(mask[i][1].shape)

        '''Check box intersections'''
        for i in range(len(predict_boxes)):
            for j in range(i + 1, len(predict_boxes)):
                if checkintersection(predict_boxes[i], predict_boxes[j]) == True:
                    mask[i][1], mask[j][1] = distinguishing(mask[i][1], mask[j][1], predict_boxes[i], predict_boxes[j])

        '''Draw Mask'''
        for i in range(len(predict_boxes)):
            result = np.array([mask[i][1], mask[i][1], mask[i][1]])
